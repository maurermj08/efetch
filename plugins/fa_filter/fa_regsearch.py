...
class FaRegSearch(IPlugin):
    ...
    def __init__(self):
        self._disply_name = 'Regex Search Bar'
        self._popularity = 0
        self._parent = True
        self._cache = False
    ...
    def get_filter(self, helper, filters):
        ...
        for key in all_filters:
            query_filter = helper.db_util.append_dict(query_filter, all_filters[key]['type'],
                    {'regexp':{all_filters[key]['regexp']:all_filters[key]['value']}})
    ...
    def get_panel_content(self, filters):
        ...
        for key in all_filters:
            title = all_filters[key]['regexp'] + ' ' + all_filters[key]['type'].replace('_',' ') + ' be "' + all_filters[key]['value'] + '"'
