...
class FaMedia(IPlugin):
    ...
    def check(self, evidence, path_on_disk):
        ...
        allowed = ['video/x-ms-asf', 'video/x-ms-asf', 'video/x-ms-asf-plugin', 'video/x-ms-asf', 'video/avi',
                   ...
                   'video/x-isvideo', 'video/mpeg', 'video/mpeg', 'video/x-motion-jpeg', 'video/quicktime',
                   'video/quicktime', 'video/x-sgi-movie', 'video/mpeg', 'video/x-mpeg', 'video/x-mpeq2a', 'video/mpeg',
    ...
    def get(self, evidence, helper, path_on_disk, request, children):
        ...
        if not os.path.isfile(newname):
            os.system(
                "avconv -i " + path_on_disk + " -c:v libx264 -c:a copy -acodec aac -strict experimental " + newname)
