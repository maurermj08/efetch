...
from plaso.output import efetch


class EfetchServer(server_config.BaseServerConfigHelper):
...
class EfetchOutputHelper(interface.ArgumentsHelper):
  ...
  def AddArguments(cls, argument_group):
    ...
    EfetchServer.AddArguments(argument_group)
  ...
  def ParseOptions(cls, options, output_module):
    ...
    """
    if not isinstance(output_module, efetch.EfetchOutputModule):
      raise errors.BadConfigObject(
          u'Output module is not an instance of EfetchOutputModule')
    ...
    EfetchServer.ParseOptions(options, output_module)
    output_module.SetCaseName(case_name)
    output_module.SetDocumentType(document_type)


manager.ArgumentHelperManager.RegisterHelper(EfetchOutputHelper)
